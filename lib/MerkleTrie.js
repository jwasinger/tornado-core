...
class MerkleTrie {
  ...
  constructor(n_levels, defaultElements, prefix, storage, hasher) {
    ...
    if (defaultElements) {
      ...
      defaultElements.forEach((element, i) => {
        this.storage.put(MerkleTrie.index_to_key(prefix, level, i), element)
      })
      level++
      ...
      for (level; level <= this.n_levels; level++) {
        for(let i = 0; i < numberOfElementsInLevel; i++) {
          const leftKey = MerkleTrie.index_to_key(prefix, level - 1, 2 * i)
          const rightKey = MerkleTrie.index_to_key(prefix, level - 1, 2 * i + 1)
          ...
          this.storage.put(MerkleTrie.index_to_key(prefix, level, i), subRoot)
        }
  ...
  async root() {
    let root = await this.storage.get_or_element(
      MerkleTrie.index_to_key(this.prefix, this.n_levels, 0),
  ...
  async path(index) {
    class PathTraverser {
      ...
      async handle_index(level, element_index, sibling_index) {
        const sibling = await this.storage.get_or_element(
          MerkleTrie.index_to_key(this.prefix, level, sibling_index),
    ...
    const element = await this.storage.get_or_element(
      MerkleTrie.index_to_key(this.prefix, 0, index),
      this.zero_values[0],
    )
    ...
    */
    try {
        ...
        async handle_index(level, element_index, sibling_index) {
          if (level == 0) {
            this.original_element = await this.storage.get_or_element(
              MerkleTrie.index_to_key(this.prefix, level, element_index),
              this.zero_values[level],
            )
          }
          const sibling = await this.storage.get_or_element(
          ...
          this.key_values_to_put.push({
            key: MerkleTrie.index_to_key(this.prefix, level, element_index),
      ...
      traverser.key_values_to_put.push({
        key: MerkleTrie.index_to_key(this.prefix, this.n_levels, 0),
  ...
  async insert(element) {
    ...
    const index = Number(element >> BigInt(256 - this.n_levels))
  ...
  getIndexByElement(element) {
      return Number(element >> BigInt(256 - this.n_levels));
  }
}
